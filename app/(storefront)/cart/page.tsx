...
export default function CartPage() {
    ...
    if (items.length === 0) {
        return (
            <div className="min-h-[70vh] flex flex-col items-center justify-center container mx-auto px-4 py-16 text-center">
                ...
                <Button asChild className="rounded-none bg-black text-white px-10 py-7 uppercase tracking-[0.15em] text-xs font-bold hover:bg-neutral-800 transition-all duration-300">
    ...
    return (
        <div className="bg-white min-h-screen pt-14 sm:pt-16 lg:pt-24 pb-40 lg:pb-12">
            <div className="container-mobile mx-auto">
                {/* Minimal Header */}
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 sm:mb-10 lg:mb-16 gap-3 sm:gap-4 border-b border-gray-100 pb-4 sm:pb-6 lg:pb-8">
                    <div>
                        <div className="flex items-center gap-4 mb-3 sm:mb-4">
                            <Link href="/shop" className="group flex items-center gap-2 text-[9px] sm:text-[10px] uppercase tracking-[0.15em] sm:tracking-[0.2em] text-gray-400 hover:text-black transition-colors inline-flex touch-target">
                                <ArrowLeft className="w-3 h-3 transition-transform group-hover:-translate-x-1" />
                                Back to Shopping
                            </Link>
                            <Link href="/" className="group flex items-center gap-2 text-[9px] sm:text-[10px] uppercase tracking-[0.15em] sm:tracking-[0.2em] text-gray-400 hover:text-black transition-colors inline-flex touch-target border-l border-gray-100 pl-4">
                                Home
                            </Link>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start relative">
                    ...
                    <div className="lg:col-span-8 space-y-6 sm:space-y-10 lg:space-y-12">
                        {items.map((item, index) => (
                            <div key={item.id || index} className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 pb-6 sm:pb-10 lg:pb-12 last:pb-0 border-b border-gray-100 last:border-0 group">
                                {/* Product Image */}
                                <div className="relative aspect-[3/4] w-full sm:w-40 lg:w-48 bg-gray-50 overflow-hidden shrink-0">
                                    <Image
                                        src={item.image}
                                        alt={item.name}
                                        fill
                                ...
                                <div className="flex flex-1 flex-col justify-between py-1">
                                        ...
                                        <div className="flex justify-between items-start gap-3 sm:gap-4">
                                            <div>
                                                <h3 className="text-base sm:text-lg lg:text-xl font-medium uppercase tracking-wide sm:tracking-wider mb-1">
                                                    <Link href={`/product/${item.productId}`} className="hover:underline underline-offset-4 decoration-1">{item.name}</Link>
                                                </h3>
                                                ...
                                                <div className="flex flex-wrap gap-x-4 sm:gap-x-6 gap-y-2 text-[10px] sm:text-xs uppercase tracking-widest text-gray-600">
                                    ...
                                    <div className="flex items-center justify-between mt-8 sm:mt-10">
                                        <div className="flex flex-col gap-2">
                                            <div className="flex items-center gap-1 border border-neutral-100 bg-white shadow-sm overflow-hidden">
                                                <button
                                                    onClick={() => updateQuantity(item.id, item.quantity - 1)}
                                                    className="p-4 sm:p-3 hover:bg-neutral-50 transition-colors disabled:opacity-20 touch-target flex items-center justify-center"
                    ...
                    <div className="lg:col-span-4 lg:sticky lg:top-20 h-fit">
                        <div className="bg-gray-50 border border-gray-100 p-5 space-y-5">
                            <h2 className="text-base font-bold uppercase tracking-[0.2em] border-b border-gray-200 pb-3">Summary</h2>
                            ...
                            <div className="space-y-2 text-xs uppercase tracking-[0.15em] font-medium">
                                ...
                                <div className="flex justify-between text-gray-500">
                                    <span>Shipping</span>
                                    <span className={cn("font-bold text-black", shippingCost === 0 && "text-green-600")}>
                                ...
                                {shippingCost > 0 && (
                                    <div className="bg-black/5 p-3 text-[9px] normal-case tracking-normal font-normal text-gray-600 leading-relaxed">
                                        <span className="font-bold uppercase tracking-widest text-[8px] block mb-1 text-black">ZODAK Premium Shipping</span>
                                        Add <span className="font-bold text-black">${(200 - subtotal).toFixed(2)}</span> for free shipping.
                                    </div>
                                )}
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold">Promo Code</p>
                                <div className="flex bg-white border border-gray-200">
                                    <input
                                        ...
                                        className="bg-transparent px-3 py-2 text-xs w-full focus:outline-none uppercase tracking-widest"
                                    />
                                    <button
                                        onClick={applyPromoCode}
                                        disabled={promoLoading}
                                        className="px-4 py-2 text-[10px] font-black uppercase tracking-widest hover:bg-gray-50 transition-colors border-l border-gray-100 shrink-0 disabled:opacity-50"
                                    >
                            ...
                            <div className="space-y-3">
                                <Button asChild className="w-full bg-black text-white hover:bg-neutral-800 rounded-none py-5 text-xs font-black uppercase tracking-[0.2em] shadow-xl hover:shadow-2xl transition-all duration-300">
                                    <Link href="/checkout">Proceed to Checkout</Link>
                                </Button>

                                <div className="flex flex-col items-center gap-3 pt-1">
                                    <div className="flex justify-center gap-3 opacity-60 grayscale hover:grayscale-0 transition-all cursor-default text-black">
                                        {/* Visa */}
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-neutral-100 p-5 safe-area-bottom z-40">
                ...
                <Button asChild className="w-full bg-black text-white rounded-none py-8 text-xs font-black uppercase tracking-[0.3em] shadow-2xl active:scale-[0.98] transition-all">
